...
let endBlock = "9402050"; // last elections as of now.. first election at 7528900
let interval = 200000;
...
async function getEvents(web3, contract, eventName) {
    try {
...
async function main() {
    ...
    for (let i = 0; i < clients.length; i++) {
        try {
            ...
            clients[i].events = await getEvents(clients[i].web3, contract, "Transfer");
